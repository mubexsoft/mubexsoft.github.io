...
const API_URL = "https://mubexsoft.github.io/mobile_api/mubex_website_api.json";
...
function openApp(app){
  ...
  modalAppName.textContent = app.app_name || "MUBEX";
  modalAppIcon.src = app.icon_path || "";
  modalAppIcon.alt = app.app_name || "MUBEX";
...
function createAppCard(app){
  ...
  card.setAttribute("aria-label", app.app_name || "MUBEX uygulaması");

  const icon = document.createElement("img");
  icon.className = "app-icon";
  icon.src = app.icon_path || "";
  icon.alt = app.app_name || "MUBEX";
  icon.loading = "lazy";
  icon.decoding = "async";

  const name = document.createElement("div");
  name.className = "app-name";
  name.textContent = app.app_name || "MUBEX";
...
async function loadApps(){
  ...
  try{
    ...
    if(!apps.length){
      appGrid.innerHTML = `<div class="empty">Gösterilecek MUBEX uygulaması bulunamadı.</div>`;
      return;
    }
    ...
    console.error("MUBEX uygulamaları yüklenemedi:",error);
